...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TPair<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			// 4. Call UGameplayStatics::ApplyDamageWithFalloff to cause damage (this will result in TakeDamage being called on the victim,
			...
			UGameplayStatics::ApplyRadialDamageWithFalloff(
				...
				DamageTypeValue,
				0.f,
				UAuraAbilitySystemLibrary::GetRaialDamageOrigin(EffectContextHandle),
				UAuraAbilitySystemLibrary::GetRaialDamageInnerRadius(EffectContextHandle),
				UAuraAbilitySystemLibrary::GetRaialDamageOuterRadius(EffectContextHandle),
				1.f,
	...
	//Armor Penetration ingnores percentage of the Target's Armor
	...
	//Armor ingnores percentage of the incoming Damage
	...
	//Capture Source Critical Hit Damage
	float SourceCriticalHitDamage = 0.f;
	ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(DamageStatics().CriticalHitDamageDef, EvaluationParameters, SourceCriticalHitDamage);
	SourceCriticalHitDamage = FMath::Max<float>(SourceCriticalHitDamage, 0.f);

	//Capture Target's Critical Hit Resistance
	...
	Damage = bCriticalHit ? (1.5f + (SourceCriticalHitDamage / 100)) * Damage : Damage;
...
void UExecCalc_Damage::DetermineDebuff(const FGameplayEffectSpec& Spec, const FGameplayEffectCustomExecutionParameters& ExecutionParams, FAggregatorEvaluateParameters& EvaluationParameters, const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		{
			// Determite if there was a successful debuff
