...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	//float F = RandomFunctionPointer(0, 0.0f, 0);
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		if (bFatal)
		{
			...
			if (CombatInterface)
			{
				FVector ImpulseDirection = UAuraAbilitySystemLibrary::GetDeathImpulse(Props.EffectContextHandle);
				CombatInterface->Die(ImpulseDirection);
			}
			SendXPEvent(Props);
		}
		else
		...
		const bool bCritcalHit = UAuraAbilitySystemLibrary::IsCriticalHit(Props.EffectContextHandle);
		ShowFloatingText(Props, LocalIncomingDamage, bBlockedHit, bCritcalHit);
